...
1. **dbacademy** creates metastore, default catalog (`neo4j_workshop`), shared warehouse
2. **workshop_data_setup.py** then:
   - Creates catalog, schemas, and UC volume
   - Uploads 22 CSV files to `/Volumes/neo4j_workshop/workshop_data/csv/`
...
### C. Genie Space (Lab 6)
Create a Genie Space pointed at `neo4j_workshop.lakehouse` tables for the Genie Agent. This is a UI-only step done in the workspace after init.

1. Go to **Genie** in the workspace sidebar
2. Click **New** to create a Genie Space
3. Name: `Aircraft Fleet Analytics`
4. Add tables: `neo4j_workshop.lakehouse.aircraft`, `systems`, `sensors`, `sensor_readings`
...
5. Verify Delta tables exist: `SELECT * FROM neo4j_workshop.lakehouse.aircraft`
...
| Volume | `/Volumes/neo4j_workshop/workshop_data/csv/` |
| Lakehouse Schema | `neo4j_workshop.lakehouse` |
