...
# ---------------------------------------------------------------------------
# 10. Full-text search (Lab 6 — keyword search via fulltext index)
# ---------------------------------------------------------------------------
...
def _fulltext_search(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  This mirrors the HybridRetriever's keyword component from Lab 6.",
    )
...
# ---------------------------------------------------------------------------
# 11. Vector search with document context (Lab 6 — VectorCypherRetriever)
# ---------------------------------------------------------------------------
...
def _vector_document_context(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  Mirrors the VectorCypherRetriever document-context pattern from Lab 6.",
    )
...
# ---------------------------------------------------------------------------
# 12. Adjacent chunk retrieval (Lab 6 — VectorCypherRetriever + NEXT_CHUNK)
# ---------------------------------------------------------------------------
...
def _adjacent_chunks(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  Mirrors the VectorCypherRetriever adjacent-chunks pattern from Lab 6.",
    )
    _cypher(_ADJACENT_Q)
    try:
        rows, _, _ = driver.execute_query(
...
# ---------------------------------------------------------------------------
# 13. Vector search → Aircraft topology (Lab 6 — system connection pattern)
# ---------------------------------------------------------------------------
...
def _vector_topology(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  Mirrors the VectorCypherRetriever system-context pattern from Lab 6.",
    )
...
# ---------------------------------------------------------------------------
# 14. Hybrid search comparison (Lab 6 — vector vs fulltext side-by-side)
# ---------------------------------------------------------------------------
...
def _hybrid_comparison(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  Demonstrates why Lab 6's HybridRetriever combines both approaches.",
    )
