...
# ---------------------------------------------------------------------------
# 10. Full-text search (Lab 4 — keyword search via fulltext index)
# ---------------------------------------------------------------------------
...
def _fulltext_search(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  This mirrors the HybridRetriever's keyword component from Lab 4.",
    )
...
# ---------------------------------------------------------------------------
# 11. Vector search with document context (Lab 4 — VectorCypherRetriever)
# ---------------------------------------------------------------------------
...
def _vector_document_context(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  Mirrors the VectorCypherRetriever document-context pattern from Lab 4.",
    )
...
# ---------------------------------------------------------------------------
# 12. Adjacent chunk retrieval (Lab 4 — VectorCypherRetriever + NEXT_CHUNK)
# ---------------------------------------------------------------------------
...
def _adjacent_chunks(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  Mirrors the VectorCypherRetriever adjacent-chunks pattern from Lab 4.",
    )
    _cypher(_ADJACENT_Q)
    try:
        rows, _, _ = driver.execute_query(
...
# ---------------------------------------------------------------------------
# 13. Vector search → Aircraft topology (Lab 4 — system connection pattern)
# ---------------------------------------------------------------------------
...
def _vector_topology(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  Mirrors the VectorCypherRetriever system-context pattern from Lab 4.",
    )
...
# ---------------------------------------------------------------------------
# 14. Hybrid search comparison (Lab 4 — vector vs fulltext side-by-side)
# ---------------------------------------------------------------------------
...
def _hybrid_comparison(driver: Driver, limit: int) -> None:
    _header(
        ...
        "  Demonstrates why Lab 4's HybridRetriever combines both approaches.",
    )
