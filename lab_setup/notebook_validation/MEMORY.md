...
The minimum viable prototype starts with 8-10 tools across four categories, with knowledge tools deferred until Lab 6 validation is complete.
...
**Deferred to Phase 2 (after Lab 6):** Knowledge tools (semantic search over maintenance manual chunks, hybrid search, diagnosis tools) and the diagnostic recommendation tool that combines user preferences with GraphRAG context. These depend on the vector indexes and entity graph that Lab 6's GraphRAG enrichment creates.
...
This prototype sits between Lab 5 (data loading, already validated by run_lab5_02.py) and Lab 7 (multi-agent supervisor). Lab 5 proves the data is in Neo4j. This prototype proves a single agent can query that data intelligently with memory. Lab 7 then composes that agent with Genie into a multi-agent system.

The prototype is phased to respect the dependency on Lab 6. Phase 1 works with only the base graph from Lab 5 plus the Lakehouse sensor tables. Phase 2, built after Lab 6 validation exists, adds GraphRAG knowledge tools for semantic search over maintenance manuals.

The scripts do not replace any existing lab content. They validate the agent patterns that Lab 7 assumes are working and provide a reusable foundation that workshop administrators can run to verify the full agent stack before participants arrive. The endpoint is admin-only; participants interact with the multi-agent supervisor in Lab 7 instead.


## Implementation Phasing

**Phase 1 (current):** Graph query tools, sensor query tool, memory tools, echo tool. Deploy agent, exercise with keyword validation, verify memory persistence. Depends on: Lab 5 data loaded, SQL warehouse available, neo4j-agent-memory wheel on cluster.

**Phase 2 (after Lab 6):** Add knowledge tools (vector search, hybrid search, diagnosis), diagnostic recommendation tool. Extend check_agent.py with GraphRAG-specific exercises. Depends on: Lab 6 GraphRAG enrichment complete (vector indexes, entity graph).
...
4. **GraphRAG dependency:** Lab 6 validation built first. Agent Phase 1 works without it; Phase 2 adds knowledge tools after Lab 6 exists.
...
13. **Audience:** Admin-only validation tool. Participants use Lab 7 multi-agent supervisor.
