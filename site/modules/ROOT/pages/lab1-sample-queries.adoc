...
TIP: All of these queries can also be run programmatically via `uv run populate-aircraft-db samples` from `lab_setup/populate_aircraft_db/`.
...
NOTE: `OPTIONAL MATCH` on `NEXT_CHUNK` keeps the last chunk in the chain (which has no successor). This shows the linked-list structure that preserves reading order.
...
NOTE: These cross-links connect the knowledge graph (extracted entities from maintenance manuals) back to the operational aircraft graph. The provenance chain traces an operating limit through its source chunk, document, and the aircraft it applies to.
...
NOTE: Multiple keywords are OR'd together by default -- chunks matching more keywords score higher.
...
NOTE: Picks a random chunk and uses its embedding to find the most similar chunks via the vector index. No API key needed -- reuses stored embeddings.
